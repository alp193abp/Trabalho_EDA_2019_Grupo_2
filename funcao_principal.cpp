...
int main (){
	...
	t1=clock()-t1;
	cout << "Tempo gasto: " << (float)t1 << " ms" << endl << endl;
	...
	ts = clock()-ts;
	cout << endl << "Tempo gasto ordenando: " << (float)ts << " ms" << endl;
	
	t0=clock()-t0;
	cout << "Tempo gasto total: " << (float)t0 << " ms" << endl;
	cout << "Ids: " << ids.size() << endl;
	cout << "Palavras unicas: " << ocorrencia.size() << endl << endl;
	clock_t t;
	while (true){
		string word;
		...
		cout << v.size() << " Resultados em " << (float)t << " ms" << endl;
	}
	return 0;
}
